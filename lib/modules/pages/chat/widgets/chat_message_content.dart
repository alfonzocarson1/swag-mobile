...
class _ContentState extends State<_Content> with AutomaticKeepAliveClientMixin {
  ...
  Widget build(BuildContext context) {
    ...
    String fileUrl =
        (isFile) ? (this.widget.message as FileMessage).secureUrl! : '';
        ...
        child: (isFile)
            ...
            : Text(
                ...
                style: TextStyle(
                  fontSize: 16,
                  color:
                      (!this.widget.isMyMessage) ? Colors.white : Colors.black,
                ),
              ),
      ),
      onTap: () => (!isImage)
  ...
  String getMessageText() {
    if (this.widget.message.data?.isNotEmpty ?? false) {
      Map<String, dynamic> messageDataJson =
          SendBirdUtils.getFormatedData(this.widget.message.data!);
      ...
      return this.widget.message.message;
  }
  ...
  bool verifyIfFileIsImage() {
    ...
    if (message.type?.contains(ChatMessageType.image.textValue) ?? false) {
      return true;
    } else if (message.type?.contains(ChatMessageType.video.textValue) ??
        false) {
      return false;
    } else
      return false;
