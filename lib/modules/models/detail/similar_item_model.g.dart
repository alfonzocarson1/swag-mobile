// GENERATED CODE - DO NOT MODIFY BY HAND

part of 'similar_item_model.dart';

// **************************************************************************
// JsonSerializableGenerator
// **************************************************************************

SimilarItemModel _$SimilarItemModelFromJson(Map<String, dynamic> json) =>
    SimilarItemModel(
      catalogItemId: json['catalogItemId'] as String,
      catalogItemImage: json['catalogItemImage'] as String,
      catalogItemName: json['catalogItemName'] as String,
      retail: json['retail'] as String,
    );

Map<String, dynamic> _$SimilarItemModelToJson(SimilarItemModel instance) =>
    <String, dynamic>{
      'catalogItemId': instance.catalogItemId,
      'catalogItemImage': instance.catalogItemImage,
      'catalogItemName': instance.catalogItemName,
      'retail': instance.retail,
    };

_$_SimilarItemModel _$$_SimilarItemModelFromJson(Map<String, dynamic> json) =>
    _$_SimilarItemModel(
      catalogItemId: json['catalogItemId'] as String,
      catalogItemImage: json['catalogItemImage'] as String,
      catalogItemName: json['catalogItemName'] as String,
      retail: json['retail'] as String,
    );

Map<String, dynamic> _$$_SimilarItemModelToJson(_$_SimilarItemModel instance) =>
    <String, dynamic>{
      'catalogItemId': instance.catalogItemId,
      'catalogItemImage': instance.catalogItemImage,
      'catalogItemName': instance.catalogItemName,
      'retail': instance.retail,
    };
